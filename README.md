...
本项目包含基于 **CARLA Simulator** (建议版本 0.9.14) 的自动驾驶行为测试和路线规划脚本。通过调用 CARLA 的 Python API 与 `GlobalRoutePlanner`，实现了从简单的路径寻点漫游，到使用基础三角函数算法进行车辆控制的进阶演练。
...
如果想在不运行模拟器的情况下查看效果，请直接播放项目根目录下的 `录像1.mp4` 和 `录像2.mp4` 文件。
